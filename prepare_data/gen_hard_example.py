...
def save_hard_example(net):
    ...
    det_boxes = cPickle.load(
        open(os.path.join(save_path, 'detections.pkl'), 'r'))
    assert len(
    ...
    for im_idx, dets, gts in zip(im_idx_list, det_boxes, gt_boxes_list):
        ...
        for box in dets:
            ...
            resized_im = cv2.resize(
                cropped_im, (image_size, image_size),
            ...
            if np.max(Iou) < 0.3:
                ...
                if np.max(Iou) >= 0.65:
                    ...
                    f1.write("%s/positive/%s" %
                             (image_size, p_idx) + ' 1 %.2f %.2f %.2f %.2f\n' %
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=['model/pnet', 'model/rnet', 'model/onet'],
        type=str)
    parser.add_argument(
        '--epoch',
        dest='epoch',
    ...
    parser.add_argument(
        ...
        action='store_true')
    parser.add_argument(
        '--vis', dest='vis', help='turn on visualization', action='store_true')
...
if __name__ == '__main__':
    ...
    test_net(args.root_path, args.dataset_path, args.image_set, args.prefix,
             args.epoch, args.batch_size, ctx, args.test_mode, args.thresh,
             args.min_face, args.stride, args.slide_window, args.shuffle,
             args.vis)
