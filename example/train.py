...
def train_net(sym,
    ...
    train_data = ImageLoader(
        imdb, net, config.BATCH_SIZE, shuffle=True, ctx=ctx)
    ...
    if initialize:
        ...
        init = mx.init.Xavier(
            factor_type="in", rnd_type='gaussian', magnitude=2)
        args = dict()
        auxs = dict()
    ...
    batch_end_callback = mx.callback.Speedometer(
        train_data.batch_size, frequent=frequent)
